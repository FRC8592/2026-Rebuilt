...
public class SparkMaxMotorExample {
    ...
    // configuration defines motor properties including inversion, soft limits,
    // limit switches, and encoder properties
    SparkMaxConfig motorConfiguration;

    // this controller will allow you to drive the motor using closed loop values
    // (i.e. position and pid control)
    SparkClosedLoopController controller;

    // the encoder on the motor will allow you to track the position of the
    // subsystem by counting ticks on the motor
    RelativeEncoder encoder;

    public SparkMaxMotorExample() {
        // In general, the motors we use will be Brushless. Brushed motors consume a lot
        // of power and are complaint-worthy
        ...
        // i.e. if the motors move diff ways and you want to match the leader motor, put
        // true
        ...
        // You can utilize different slots(ex. slot1, slot2) to create different pids
        // for the same motor
        // i.e. different pid values for an arm holding heavy objects vs. with no
        // objects
        motorConfiguration.closedLoop.pid(0, 0, 0, ClosedLoopSlot.kSlot0);

        // APPLY CONFIGURATION TO MOTOR - MUST DO WHENEVER YOU CHANGE CONFIGURATION
        // SETTINGS
    ...
    public void runVelocityControl() {
        // the setControl method utilizes closed loop (meaning that it will use the pid
        // values)
        // other methods such as setVoltage() and set() will NOT use the closed loop
        // settings

        // the setpoint value is in RPM
        // ControlType is velocity, which is best for when you want to run a motor up to
        // a set velocity and position doesn't matter
    ...
    public void runPositionControl() {
        ...
        // ControlType is position, which is best for position-based mechanisms (arm,
        // turret, etc)
