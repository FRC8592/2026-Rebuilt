...
public class TalonFXMotorExample {
    ...
    // configurations for every motor!
    private TalonFX followerMotor = new TalonFX(10);

    // configuration defines motor properties including inversion, pid, and current
    // limits
    ...
    // you should use this when you care about the motor speed but not which
    // position it ends at
    // i.e. flywheels, conveyer belts
    private VelocityVoltage velocityRequest = new VelocityVoltage(0);

    // this is the controller that can drive the motor using motion magic, and also
    // uses FOC (field-oriented control)
    ...
    public TalonFXMotorExample() {
        ...
        // use MotorAlignmentValue.Opposed when the motors spin in the opposite
        // direction
        ...
        // Motion Magic provides smoother acceleration/deacceleration, and can build on
        // top of pid or be used independently
        // this will provide trapezoidal profile (if jerk is zero) or an s-curve profile
        // (if jerk is non-zero)
        ...
        // if you're using position control, you might want to zero its position when
        // you start the robot
        motor.setPosition(0);
